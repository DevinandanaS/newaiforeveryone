...
class Title {
  ...
  createMesh() {
    ...
    // Adjust scale based on aspect ratio to fit within the plane
    const planeWidth = this.plane.scale.x;
    const planeHeight = this.plane.scale.y;
    
    let textWidth = planeWidth * 0.85;
    let textHeight = textWidth / aspect;
    
    if (textHeight > planeHeight * 0.5) {
        textHeight = planeHeight * 0.5;
        textWidth = textHeight * aspect;
    }

    this.mesh.scale.set(textWidth, textHeight, 1);
    // Position text at the bottom half of the card
    this.mesh.position.y = -planeHeight * 0.25; 
